  ...
  return (
    <AppShell currentStage={3} rightPanel={copilotPanel}>
      <div className="p-8 max-w-5xl">
        <div className="mb-7">
          <div className="text-xs font-bold text-[#4a6080] uppercase tracking-wider mb-1">Stage 3. Common Analysis</div>
          <h1 className="text-3xl font-black text-[#004999] tracking-tight">기본 데이터 분석 및 사실 검증</h1>
          <p className="text-sm text-[#4a6080] mt-1.5 font-medium">추가 자료 수집, 검색, 공통 분석, 세부 조건 추출을 수행하고 브랜치 출력으로 넘깁니다.</p>
        </div>
        ...
        <div className="mb-6 border border-[#004999] bg-[#004999] p-4 text-[#ffffff]">
          ...
          <div className="text-xs text-[#dce8f0] mt-1">이 분기는 Stage 2에서 확정되었으며 이 화면에서는 변경할 수 없습니다.</div>
        </div>

        <div className="grid grid-cols-3 gap-6">
          <div className="space-y-5">
            <div className="border border-[#6b8199] bg-[#f0f5fa] p-4">
              <div className="flex items-center gap-2 mb-3">
                <span className="material-symbols-outlined text-[18px] text-[#4a6080]">upload_file</span>
                <h3 className="text-sm font-bold text-[#004999]">A. Additional Materials</h3>
              </div>
              <div className="space-y-2">
                {REQUIRED_DOCS.map((item) => (
                  <div key={item.label} className="flex items-center gap-2">
                    <div className={`w-3 h-3 flex items-center justify-center ${item.done ? "bg-[#004999]" : "border border-[#6b8199]"}`}>
                      {item.done && <span className="material-symbols-outlined text-[9px] text-[#ffffff]">check</span>}
                    </div>
                    <span className={`text-[10px] ${item.done ? "text-[#004999] font-medium" : "text-[#6b8199]"}`}>{item.label}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="border border-[#b8c8d8] bg-[#ffffff] p-4">
              <div className="flex items-center gap-2 mb-3">
                <span className="material-symbols-outlined text-[16px] text-[#4a6080]">search</span>
                <h3 className="text-sm font-bold text-[#004999]">B. Reference Search Results</h3>
              </div>
              <div className="space-y-2">
                {[
                  "유사 사례 3건 선택됨",
                  ...
                  <div key={item} className="border border-[#dce8f0] px-3 py-2 text-xs text-[#0d1b2a] bg-[#f5f8fc]">
                    {item}
                  </div>
                ))}
              </div>
          ...
          <div className="col-span-2 space-y-5">
            <div>
              <h3 className="text-sm font-bold text-[#004999] mb-3 border-b border-[#b8c8d8] pb-2">C. Common Analysis Completion</h3>
              <div className="grid grid-cols-2 gap-3">
                {ANALYSIS_ITEMS.map((item) => (
                  <div key={item.label} className={`border p-4 ${item.done ? "border-[#b8c8d8] bg-[#ffffff]" : "border-[#004999] bg-[#f5f8fc]"}`}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-bold text-[#004999]">{item.label}</span>
                      <span className="material-symbols-outlined text-[18px] text-[#4a6080]">{item.icon}</span>
                    </div>
                    <span className={`text-[10px] font-bold ${item.done ? "text-[#004999]" : "text-[#4a6080]"}`}>{item.done ? "완료" : item.status}</span>
                  </div>
                ))}
              </div>
              <button
                ...
                className={`mt-4 px-4 py-2 text-xs font-bold transition-colors ${
                  state.commonAnalysisReady ? "border border-[#6b8199] text-[#004999]" : "bg-[#004999] text-[#ffffff] hover:bg-[#1a3a5c]"
                }`}
              >
            ...
            <div className="border border-[#b8c8d8] bg-[#ffffff] p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold text-[#004999]">D. Extracted Deal Terms</h3>
                <span className="px-2 py-0.5 text-[10px] font-bold bg-[#dce8f0] text-[#004999]">EXTRACTED</span>
              </div>
              <div className="space-y-2">
                <div className="border-l-2 border-[#004999] pl-3 py-1">
                  <div className="text-xs font-bold text-[#004999]">상환 일정</div>
                  <div className="text-[11px] text-[#4a6080] mt-0.5">2024-06-30 (10%), 2024-12-31 (20%), 2025-12-31 (70%)</div>
                </div>
                <div className="border-l-2 border-[#004999] pl-3 py-1">
                  <div className="text-xs font-bold text-[#004999]">표면 금리 / 실효 금리</div>
                  <div className="text-[11px] text-[#4a6080] mt-0.5">표면 5.5% / 실효 6.2%</div>
                </div>
              </div>
            </div>

            <div className="border border-[#6b8199] bg-[#f0f5fa] p-4">
              <h3 className="text-sm font-bold text-[#004999] mb-2">E. Output Gate</h3>
              <div className="text-xs text-[#4a6080] mb-3">
        ...
        <div className="flex items-center justify-between pt-4 mt-6 border-t border-[#b8c8d8]">
          ...
          <button
            ...
            className={`px-6 py-2.5 text-sm font-bold transition-colors ${
              canAdvanceFromStep3 ? "bg-[#004999] text-[#ffffff] hover:bg-[#1a3a5c]" : "border border-[#b8c8d8] text-[#6b8199] cursor-not-allowed"
            }`}
          >
            {nextLabel}
          </button>
