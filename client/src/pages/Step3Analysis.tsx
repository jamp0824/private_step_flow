      ...
      <div className="p-8 max-w-5xl">
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="space-y-5">
            <div className="border border-[#777777] bg-[#f3f3f3] p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-bold text-[#000000]">A. Additional Materials</h3>
              </div>
              <div className="space-y-2">
                {REQUIRED_DOCS.map((item) => (
                  <div key={item.label} className="flex items-center gap-2">
                    <div className={`w-3 h-3 flex items-center justify-center ${item.done ? "bg-[#000000]" : "border border-[#777777]"}`}>
                      {item.done && <span className="material-symbols-outlined text-[9px] text-[#ffffff]">check</span>}
                    </div>
                    <span className={`text-[10px] ${item.done ? "text-[#000000] font-medium" : "text-[#777777]"}`}>{item.label}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="border border-[#c6c6c6] bg-[#ffffff] p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-bold text-[#000000]">B. Reference Search Results</h3>
              </div>
              <div className="space-y-2">
                {[
                  "유사 사례 3건 선택됨",
                  ...
                  <div key={item} className="border border-[#e2e2e2] px-3 py-2 text-xs text-[#1a1c1c] bg-[#f9f9f9]">
                    {item}
                  </div>
                ))}
              </div>
          ...
          <div className="col-span-2 space-y-5">
            <div>
              <h3 className="text-sm font-bold text-[#000000] mb-3 border-b border-[#c6c6c6] pb-2">C. Common Analysis Completion</h3>
              <div className="grid grid-cols-2 gap-3">
                {ANALYSIS_ITEMS.map((item) => (
                  <div key={item.label} className={`border p-4 ${item.done ? "border-[#c6c6c6] bg-[#ffffff]" : "border-[#000000] bg-[#f9f9f9]"}`}>
                    ...
                    <span className={`text-[10px] font-bold ${item.done ? "text-[#000000]" : "text-[#5e5e5e]"}`}>{item.done ? "완료" : item.status}</span>
                  </div>
                ))}
              </div>
              <button
            ...
            <div className="border border-[#c6c6c6] bg-[#ffffff] p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold text-[#000000]">D. Extracted Deal Terms</h3>
                <span className="px-2 py-0.5 text-[10px] font-bold bg-[#e2e2e2] text-[#000000]">EXTRACTED</span>
              </div>
              <div className="space-y-2">
                <div className="border-l-2 border-[#000000] pl-3 py-1">
                  <div className="text-xs font-bold text-[#000000]">상환 일정</div>
                  <div className="text-[11px] text-[#5e5e5e] mt-0.5">2024-06-30 (10%), 2024-12-31 (20%), 2025-12-31 (70%)</div>
                </div>
                <div className="border-l-2 border-[#000000] pl-3 py-1">
                  <div className="text-xs font-bold text-[#000000]">표면 금리 / 실효 금리</div>
                  <div className="text-[11px] text-[#5e5e5e] mt-0.5">표면 5.5% / 실효 6.2%</div>
                </div>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between pt-4 mt-6 border-t border-[#c6c6c6]">
          ...
          <button
            ...
            className={`px-6 py-2.5 text-sm font-bold transition-colors ${
              canAdvanceFromStep3 ? "bg-[#000000] text-[#ffffff] hover:bg-[#3a3c3c]" : "border border-[#c6c6c6] text-[#777777] cursor-not-allowed"
            }`}
          >
            {nextLabel}
          </button>
