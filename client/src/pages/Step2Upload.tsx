...
function StatusBadge({ tone, label }: { tone: "dark" | "light" | "warn" | "error"; label: string }) {
  const className =
    tone === "dark"
      ? "border border-[#004999] bg-[#004999] text-[#ffffff]"
      : tone === "warn"
                      ...
                      <td className="px-4 py-3">
                        {doc.parseStatus === "error" && !state.parseErrorResolved ? (
                          <button
                            ...
                            className="px-3 py-1 border border-[#c0392b] text-[10px] font-bold text-[#c0392b] hover:bg-[#fff5f5] transition-colors"
                          >
                            재파싱 완료 표시
                          </button>
                        ) : (
              ...
              <div className="space-y-2">
                {BASE_CHECKLIST.map((item) => (
                  <div key={item} className="flex items-center gap-2">
                    <div className="w-4 h-4 bg-[#004999] flex items-center justify-center">
                      <span className="material-symbols-outlined text-[11px] text-[#ffffff]">check</span>
                    </div>
                    <span className="text-xs text-[#004999] font-medium">{item}</span>
                  </div>
                ))}
              </div>
            </div>
