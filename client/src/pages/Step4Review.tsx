...
function RiskLevelBadge({ level }: { level: string }) {
  ...
  if (level === "MODERATE") return <span className="px-2 py-0.5 text-[10px] font-bold border border-[#6b8199] text-[#004999]">MODERATE</span>;
  ...
  return (
    <AppShell currentStage={4} rightPanel={copilotPanel}>
      <div className="p-8 max-w-5xl">
        <div className="mb-7">
          <div className="text-xs font-bold text-[#4a6080] uppercase tracking-wider mb-1">Stage 4. Integrated Review / Human Confirm</div>
          <h1 className="text-3xl font-black text-[#004999] tracking-tight">종합 판단 및 사람 확인</h1>
          <p className="text-sm text-[#4a6080] mt-1.5 font-medium">공통 분석과 브랜치 분석을 병합하고 인간 판단으로 결과를 확정합니다.</p>
        </div>
        ...
        <div className="mb-6 border border-[#004999] bg-[#004999] p-4 text-[#ffffff]">
          ...
          <div className="text-xs text-[#dce8f0] mt-1">Carry-forward issues: {branchIssues.join(" / ")}</div>
        </div>

        <div className="grid grid-cols-3 gap-4 mb-6">
          {RISK_METRICS.map((metric) => (
            <div key={metric.label} className="border border-[#b8c8d8] bg-[#ffffff] p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-bold text-[#4a6080] uppercase tracking-wide">{metric.label}</span>
                <RiskLevelBadge level={metric.level} />
              </div>
              <div className="text-2xl font-black text-[#004999] tracking-tight mb-1">{metric.value}</div>
              <div className="text-[10px] text-[#6b8199] leading-relaxed">{metric.desc}</div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="col-span-2 space-y-5">
            <div className="border border-[#b8c8d8] bg-[#ffffff] p-4">
              <h2 className="text-sm font-bold text-[#004999] mb-3">A. Integrated Analysis</h2>
              <div className="space-y-3 text-xs text-[#0d1b2a]">
                <div className="border border-[#dce8f0] p-3 bg-[#f5f8fc]">계약조건 전반은 시장 표준과 유사하나, 스프레드 및 구조 리스크가 추가 확인 대상입니다.</div>
                <div className="border border-[#dce8f0] p-3 bg-[#f5f8fc]">브랜치 특화 분석은 Stage 4의 human confirm 없이는 최종 판단으로 취급되지 않습니다.</div>
              </div>
            </div>

            <div className="border border-[#004999] p-5">
              <h2 className="text-sm font-bold text-[#004999] mb-3">B. Human Confirmation</h2>
              <div className="grid grid-cols-4 gap-3 mb-4">
                <button
                  ...
                  className={`py-2 text-xs font-bold transition-colors ${state.decisionStatus === "confirmed" ? "bg-[#004999] text-[#ffffff]" : "border border-[#6b8199] text-[#004999] hover:bg-[#f0f5fa]"}`}
                >
                  확정
                </button>
                <button
                  ...
                  className={`py-2 text-xs font-bold transition-colors ${state.decisionStatus === "modified" ? "bg-[#004999] text-[#ffffff]" : "border border-[#6b8199] text-[#004999] hover:bg-[#f0f5fa]"}`}
                >
                  수정 후 확정
                </button>
                <button
                ...
                <button
                  ...
                  className={`py-2 text-xs font-bold transition-colors ${state.decisionStatus === "supplement_requested" ? "bg-[#004999] text-[#ffffff]" : "border border-[#6b8199] text-[#004999] hover:bg-[#f0f5fa]"}`}
                >
                  보완 요청
                </button>
              </div>
              <textarea
                ...
                className="w-full border border-[#6b8199] bg-[#ffffff] p-3 text-xs text-[#0d1b2a] placeholder-[#6b8199] outline-none focus:border-[#004999] resize-none"
                rows={4}
          ...
          <div className="space-y-5">
            <div className="border border-[#b8c8d8] bg-[#ffffff] p-4">
              <h2 className="text-sm font-bold text-[#004999] mb-3">C. Collaboration / Conditions</h2>
              <div className="space-y-2 text-xs">
                <div className="border border-[#dce8f0] p-3 bg-[#f5f8fc]">보완 요청 상태: {state.supplementStatus}</div>
                <div className="border border-[#dce8f0] p-3 bg-[#f5f8fc]">조건 변경 이력: 아직 없음</div>
                <div className="border border-[#dce8f0] p-3 bg-[#f5f8fc]">공유 메모: 법무팀 협의 필요 여부 검토</div>
              </div>
            </div>

            <div className="border border-[#b8c8d8] bg-[#ffffff] p-4">
              <div className="text-[10px] font-bold text-[#4a6080] uppercase tracking-wide mb-2">Decision State</div>
              <div className="text-sm font-black text-[#004999] mb-1">{state.decisionStatus.toUpperCase()}</div>
          ...
          <button
            ...
            className={`px-6 py-2.5 text-sm font-bold transition-colors ${
              canAdvanceFromStep4 || state.decisionStatus === "supplement_requested"
                ? "bg-[#004999] text-[#ffffff] hover:bg-[#1a3a5c]"
                : "border border-[#b8c8d8] text-[#6b8199] cursor-not-allowed"
            }`}
          >
